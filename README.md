...
- Lightweight editor status rendering keeps expensive session, git, and quota refreshes outside the typing hot path
...
### Editor status performance model

Pi owns the native editor, widget, and footer layout. pi-coder-theme only renders editor chrome/status labels and keeps non-critical status fields such as git summary, token usage, cost, and quota in cached snapshots refreshed asynchronously outside the editor render and lifecycle-event hot paths.
