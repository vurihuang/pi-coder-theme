...
function matchesScrollShortcut(data: string, shortcut: string): boolean {
...
function columnSegments(text: string): Array<{ text: string; startCol: number; endCol: number }> {
  let col = 0;
  const segments: Array<{ text: string; startCol: number; endCol: number }> = [];
  for (const { segment } of graphemeSegmenter.segment(text)) {
    const width = Math.max(0, visibleWidth(segment));
    segments.push({ text: segment, startCol: col, endCol: col + width });
    col += width;
  }
  return segments;
}

function selectionSegmentRange(line: string, col: number): { startCol: number; endCol: number } | null {
  const segments = columnSegments(line);
  const index = segments.findIndex((segment) => col >= segment.startCol && col < segment.endCol);
  if (index === -1 || /^\s$/u.test(segments[index]?.text ?? "")) return null;

  let startIndex = index;
  while (startIndex > 0 && !/^\s$/u.test(segments[startIndex - 1]?.text ?? "")) {
    startIndex -= 1;
  }

  let endIndex = index;
  while (endIndex + 1 < segments.length && !/^\s$/u.test(segments[endIndex + 1]?.text ?? "")) {
    endIndex += 1;
  }

  return {
    startCol: segments[startIndex]?.startCol ?? col,
    endCol: segments[endIndex]?.endCol ?? col,
  };
}
...
export class TerminalSplitCompositor {
  ...
  setClusterSuppressed(suppressed: boolean): void {
    if (this.disposed || this.clusterSuppressed === suppressed) return;
    if (suppressed) {
      this.clearPaintedCluster();
      this.originalWrite(beginSynchronizedOutput() + hideCursor() + endSynchronizedOutput());
    }
    this.clusterSuppressed = suppressed;
    this.requestRender();
  }

  requestRepaint(): void {
    if (this.disposed || this.clusterSuppressed || this.hasVisibleOverlay()) return;
  ...
  clearPaintedCluster(): void {
    ...
    const rawRows = this.getRawRows();
    const width = Math.max(1, this.terminal.columns || 80);
    const clusterLineCount = Math.max(
      this.lastPaintedClusterLineCount,
      this.visibleClusterLines.length,
      this.getCluster(width, rawRows).lines.length,
    );
    if (clusterLineCount === 0) return;

    const startRow = Math.max(1, rawRows - clusterLineCount + 1);
    let buffer = beginSynchronizedOutput() + resetScrollRegion();
    for (let i = 0; i < clusterLineCount; i++) {
      buffer += moveCursor(startRow + i, 1) + clearLine();
    }
  ...
  private getScrollableRows(): number {
    if (this.disposed || this.clusterSuppressed || this.writing || this.renderingCluster || this.checkingOverlay || this.hasVisibleOverlay()) {
  ...
  private startSelection(location: SelectionLocation): void {
    ...
    if (
      ...
      const lineText = this.selectionLineText(location.area, line);
      const range = selectionSegmentRange(lineText, location.point.col);
      if (!range) {
        this.clearSelection();
        this.lastLeftPress = null;
        this.requestRender();
      ...
      this.selectionAnchor = { line, col: range.startCol };
      this.selectionFocus = { line, col: range.endCol };
  ...
  private getCluster(width: number, terminalRows: number): FixedEditorClusterRender {
    if (this.clusterSuppressed) return { lines: [], cursor: null };
